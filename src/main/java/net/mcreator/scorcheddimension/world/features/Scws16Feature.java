...
public class Scws16Feature extends Feature<NoneFeatureConfiguration> {
	...
	public static Feature<?> feature() {
		...
		CONFIGURED_FEATURE = FeatureUtils.register("scorcheddimension:scws_16", FEATURE, FeatureConfiguration.NONE);
		PLACED_FEATURE = PlacementUtils.register("scorcheddimension:scws_16", CONFIGURED_FEATURE, List.of());
		return FEATURE;
	}
	...
	public boolean place(FeaturePlaceContext<NoneFeatureConfiguration> context) {
		if (!generate_dimensions.contains(context.level().getLevel().dimension()))
			return false;
		if (template == null)
			template = context.level().getLevel().getStructureManager().getOrCreate(new ResourceLocation("scorcheddimension", "scorchedwoodstr16"));
		if (template == null)
			return false;
