...
public class Scws19Feature extends Feature<NoneFeatureConfiguration> {
	...
	public static Feature<?> feature() {
		...
		CONFIGURED_FEATURE = FeatureUtils.register("scorcheddimension:scws_19", FEATURE, FeatureConfiguration.NONE);
		PLACED_FEATURE = PlacementUtils.register("scorcheddimension:scws_19", CONFIGURED_FEATURE, List.of());
		return FEATURE;
	}
	...
	public boolean place(FeaturePlaceContext<NoneFeatureConfiguration> context) {
		if (!generate_dimensions.contains(context.level().getLevel().dimension()))
			return false;
		if (template == null)
			template = context.level().getLevel().getStructureManager().getOrCreate(new ResourceLocation("scorcheddimension", "scorchedwoodstr19"));
		if (template == null)
			return false;
